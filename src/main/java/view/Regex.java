...
public class Regex {
   ...
   public static String removeCardMain = "deck rm-card (?=.*--card (?<cardName>.+)) (?=.*--deck (?<deckName>\\S+))";
    public static String removeCardSide = "deck rm-card (?=.*--card (?<cardName>.+)) (?=.*--deck (?<deckName>\\S+))"+
            "(?=.*--(?<side>side))";
